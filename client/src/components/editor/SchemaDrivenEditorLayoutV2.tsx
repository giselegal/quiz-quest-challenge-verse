...
const SchemaDrivenEditorLayoutV2: React.FC<SchemaDrivenEditorLayoutV2Props> = ({ 
  ...
  // Handlers para criar páginas específicas do funil
  const createQuizStartPage = () => {
    const newPage = {
      title: 'Página Inicial do Quiz',
      slug: `quiz-start-${Date.now()}`,
      blocks: [
        {
          id: `quiz-intro-${Date.now()}`,
          type: 'QuizIntroHeaderBlock',
          properties: {
            title: 'Descubra Seu Estilo Único',
            subtitle: 'Quiz Personalizado',
            description: 'Responda algumas perguntas e descubra qual estilo combina mais com você',
  ...
  const createQuizQuestionPage = () => {
    const questionNumber = (funnel?.pages || []).filter(p => 
      p.title.includes('Questão') || p.blocks.some(b => b.type === 'QuizQuestionBlock')
    ).length + 1;
    
    const newPage = {
      title: `Questão ${questionNumber}`,
      slug: `quiz-question-${questionNumber}-${Date.now()}`,
      blocks: [
        {
          id: `quiz-question-${Date.now()}`,
          type: 'QuizQuestionBlock',
          properties: {
            question: `Pergunta ${questionNumber}:`,
            questionText: 'Digite sua pergunta aqui...',
            options: [
              { id: 'opt1', text: 'Opção 1', points: { resultado1: 1, resultado2: 0 } },
              { id: 'opt2', text: 'Opção 2', points: { resultado1: 0, resultado2: 1 } }
            ],
            type: 'single'
          }
        }
      ...
      blocks: [
        {
          ...
          properties: {
            title: 'Seu Resultado:',
            description: 'Baseado nas suas respostas, aqui está seu perfil único',
            results: [
              {
                id: 'resultado1',
                title: 'Perfil Elegante',
  ...
  const createLandingPage = () => {
    const newPage = {
      ...
      blocks: [
        ...
        {
          id: `landing-description-${Date.now()}`,
          type: 'text-inline',
          properties: {
            text: 'Descubra mais sobre você mesmo com nosso quiz personalizado.',
            size: 'medium',
            color: '#6b7280'
          }
        },
        {
          id: `landing-cta-${Date.now()}`,
          type: 'cta-inline',
          properties: {
            text: 'Começar Agora',
            style: 'primary',
